...
public static class AzureIocExtensions
{
        ...
        Action<IdempotencyConfigurationBuilder>? configureIdempotency = null)
    {
        ...
        else
        {
            ...
            // Must be a singleton so the in-memory dedup store persists across messages
            // (a scoped instance would be recreated per message scope, defeating deduplication
            // and acting as a captive dependency for the singleton listeners).
